...
{
	data->philo_nb = (int)ft_stol(argv[1]);
	data->time_die = (int)ft_stol(argv[2]);
	data->time_eat = (int)ft_stol(argv[3]);
	data->time_sleep = (int)ft_stol(argv[4]);
	data->time_think = 0;
	...
	if (argc == 5)
		data->must_eat = -1;
	if (argc == 6)
		data->must_eat = (int)ft_stol(argv[5]);
	data->done = FALSE;
	data->died = FALSE;
